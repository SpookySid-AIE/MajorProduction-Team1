...
public class playerPossession : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if (true) //hit.transform.tag == "Item")
            {
        ...
        { 
            if (hit.transform.tag == "Item")
            {
                if (player.GetComponent<playerController>().Ectoplasm > 0.0f)
                {
                    ...
                    //Set up the follow distances
                    var smooth = Camera.main.gameObject.GetComponent<SmoothFollowWithCameraBumper>();
                    var itemToBePossessed = target.GetComponent<ItemController>();
                    smooth.distance = itemToBePossessed.distance;
                    smooth.targetLookAtOffset = itemToBePossessed.targetOffset;
                    smooth.target = target.transform;
